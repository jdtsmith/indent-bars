...
- Support ~yaml-pro-ts-mode~ and modes like ~typescript-tsx-mode~ derived from ~typescript-ts-base-mode~.
- Work correctly with ~whitespace-mode~ and other modes which change the character display tables to ensure display spaces for blank lines are not visible.
- Implement bar-skipping in multiply nested list contexts (with ~indent-bars-no-descend-lists~ set).
